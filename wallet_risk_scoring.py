...
def get_supply_borrow(wallet):
    ...
    if "data" not in json_data or json_data["data"]["account"] is None:
        print(f"No data for wallet {wallet}")
        return 0.0, 0.0

    account = json_data["data"]["account"]
    supply_usd, borrow_usd = 0.0, 0.0

    for token in account["tokens"]:
        try:
            supply = float(token["supplyBalanceUnderlying"])
            borrow = float(token["borrowBalanceUnderlying"])
            price = float(token["market"]["underlyingPriceUSD"])
            supply_usd += supply * price
            borrow_usd += borrow * price
        except (TypeError, ValueError):
            continue

    return supply_usd, borrow_usd

def get_liquidations(wallet):
    query = """
    {
      liquidates(where: {borrower: "%s"}) {
        id
      }
    }
    """ % wallet.lower()
    ...
    json_data = response.json()
    if "data" not in json_data or json_data["data"].get("liquidates") is None:
        return 0

    liquidates = json_data["data"]["liquidates"]
    return len(liquidates)

results = []
for wallet in tqdm(wallets):
    supply_usd, borrow_usd = get_supply_borrow(wallet)
    liquidations = get_liquidations(wallet)

    repayment_ratio = np.random.uniform(0.7, 1.0)
    wallet_age_years = np.random.uniform(0.5, 4)

    results.append({
        "wallet_id": wallet,
        "supplied_usd": supply_usd,
...
scaler = MinMaxScaler()
df[["supplied_usd_norm", "borrowed_usd_norm"]] = scaler.fit_transform(
    df[["supplied_usd", "borrowed_usd"]]
)

liq_max = max(1, df["liquidations"].max())
df["liquidations_norm"] = 1 - (df["liquidations"] / liq_max)
df["wallet_age_norm"] = df["wallet_age_years"] / df["wallet_age_years"].max()
...
df["score"] = (
    W_SUPPLY * df["supplied_usd_norm"]
    + W_BORROW * df["borrowed_usd_norm"]
    + W_LIQ * df["liquidations_norm"]
    + W_REPAY * df["repayment_ratio"]
    + W_AGE * df["wallet_age_norm"]
)

df["score"] = df["score"].clip(lower=0, upper=1000).round().astype(int)

print("\n✅ Final scores:")
print(df[["wallet_id", "score"]])

df[["wallet_id", "score"]].to_csv("wallet_risk_scores.csv", index=False)
print("\n✅ CSV saved: wallet_risk_scores.csv")
